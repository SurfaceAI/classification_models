...
def run_image_per_image_predict_segmentation(config):
    ...
    for image, image_id in predict_data:
        ...
        if count > 20:
            break
        # detection
        detections = mapillary_requests.extract_detections_from_image(mapillary_requests.request_image_data_from_image_entity(image_id, 
                                         mapillary_requests.load_mapillary_token(token_path=config.get('mapillary_token_path')), 
                                         url=False, 
                                         detections=True))
        ...
        for det in detections:
            if det['value'] in config.get('segment_color').keys():
                ...
                rescaled_segments = [np.divide(segment, 4096) for segment in mapillary_detections.decode_detection_geometry(det['geometry'])]

                detection_polygons = [mapillary_detections.convert_to_polygon(segment) for segment in rescaled_segments]
                merged_polygon = mapillary_detections.merge_polygons(detection_polygons)

                # reject detection if coverage area of detection is low
                if mapillary_detections.calculate_polygon_area(merged_polygon) < 0.05:
                    continue
                
                convex_hull = mapillary_detections.generate_polygon_convex_hull(merged_polygon)
                bbox = mapillary_detections.generate_polygon_bbox(convex_hull)
                # image_crop = crop_image_by_bbox(image, bbox)
                # left, upper=lower, right, lower =upper
                # traqnsform : top, left, height, width
                top = 1 - bbox[3]
                left = bbox[0]
                ...
                rescaled_convex = np.multiply(convex_hull.exterior.coords, image.size).flatten().tolist()
                ...
                text = recursive_image_per_image_predict_csv(model_dict=config.get("model_dict"), model_root=config.get("root_model"), image_id=image_id, segment=det['value'], image_transformed=image_transformed, device=device, df=df, level=level)
                ...
                draw_det.polygon(np.multiply(merged_polygon.exterior.coords, image_det.size).flatten().tolist(), fill=config.get('segment_color')[det['value']], outline="blue")
                # draw bbox
                left = bbox[0] * image_det.size[0]
                right = bbox[2] * image_det.size[0]
                top = bbox[1] * image_det.size[1]
                upper = bbox[3] * image_det.size[1]
                draw_det.rectangle([left, top, right, upper], outline=config.get('segment_color')[det['value']])
        ...
        composite = Image.blend(image, image_det, 0.1)
...
def recursive_image_per_image_predict_csv(model_dict, model_root, image_id, segment, image_transformed, device, df, level, pre_cls=None):
    ...
    if model_dict is None:
        ...
        pred_output = predict_image_per_image(model, image_transformed, is_regression, device)
        ...
        columns = ['Image', 'Segment', 'Prediction', 'is_in_validation', f'Level_{level}'] # is_in_valid_dataset / join
        pre_cls_entry = []
        if pre_cls is not None:
            columns = columns + [f'Level_{level-1}']
            pre_cls_entry = [pre_cls]
        if is_regression:
            cls = "outside" if str(pred_output.round().int().item()) not in classes.keys() else classes[str(pred_output.round().int().item())]
            i = df.shape[0]
            df.loc[i, columns] = [image_id, segment, pred_output.item(), is_valid_data, cls] + pre_cls_entry
            ...
            for cls, prob in zip(classes, pred_output.tolist()):
                i = df.shape[0]
                df.loc[i, columns] = [image_id, segment, prob, is_valid_data, cls] + pre_cls_entry
            # subclasses not for regression implemented
            sub_model_dict = model_dict.get('submodels', {}).get(pred_class)
            sub_text = recursive_image_per_image_predict_csv(model_dict=sub_model_dict, model_root=model_root, image_id=image_id, segment=segment, image_transformed=image_transformed, device=device, df=df, level=level+1, pre_cls=cls)
            text = pred_class + ': ' + f"{torch.max(pred_output, dim=0).values.item():.3f}" + '\n' + sub_text
