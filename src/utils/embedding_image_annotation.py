...
# 1. Unpickle error auf dem Server 'RuntimeError: Attempting to deserialize object on a CUDA device but torch.cuda.is_available() is False. 
...
# -> Habe die embeddings in den image_annotation ordner bei mir verlinkt, eig müssten die Pfade stimmen

# 2. Es fehlt der code, der die results mit dem optimal threshold speichert:
# Ich denke gerade, dass es einfacher wäre erst die thresholds zu berechnen und zu speichern. Dann für alle 5 Klassen zu laden und die search nochmal neu durchlaufen zu lassen.
# Vielleicht findest du aber auch einen einfacheren Weg die results bereits in dem 'sweep' zu speichern, wäre natürlich eigentlich sinnvoller.

# 3. Filtern des df nach den 50 Bildern für das experiment 0 (wie bei GPT). Diese werden dann statt df[condition] als 'one-shot' Bilder verwendet. 
# Die csv mit den image id's habe ich leider nur bei mir lokal, ich kann die aber gleich mal auf den server kopieren.
# Oder würdest du das anders machen? (images_experiment_0_asphalt.csv,  images_experiment_0_paving_stones.csv)

# Ist das embeddings filtern für V101 und V200 mit 'change_indices' umständlich, gibt es da eine Variante die automatisch alle 3 embeddings nach type filtern kann (position id)? -> Falls Zeit dafür ist

# 4. Allgeminer clean up, was du so für notwendig hälst.

# 5. Wenn alles steht für alle 5 conditions Experimente 0 (V11, 50), 1 (V101), 2 (V200) durchlaufen lassen 

# 6. Ggf. noch auf anderen embeddings laufen lassen ? 
...
#config
# TODO: embeddings TYPE (clip, effnet...) and loop
query_dataset = 'V12' # fixed
# corpus_dataset = 'V12'
embeddings_model_list = ['clip', 'dino', 'effnet']
# embeddings_model_list = ['dino']
# is_threshold_given = False # Experiment 0: False, Experiment 1 & 2: True
...
# threshold_file_prefix = 'threshold_'
# experiment_prefix = 'find_threshold'
gpu_kernel = 0

root_path = str(Path().resolve()),
experiments_path = os.path.join(root_path, "annotation_experiments")
gpt_results_path = os.path.join(root_path, "annotation_experiments", "gpt_results")
embeddings_path = os.path.join(root_path, 'embeddings')
...
for experiment in experiments_list:
    ...
    for embeddings_model in embeddings_model_list:
        ...
        for condition in conditions_list:
            ...
            df[['type', 'quality', 'image_id']] = df.apply(lambda x: helper.extract_type_and_quality_and_id_from_img_path(x["path"]), axis=1, result_type='expand')
            ...
            # # code dump from GPT experiments to filter the 50 images/class
            # # TODO: hier conditions definieren, anstatt später?
            # query_images_list = pd.read_csv(os.path.join(experiments_path, query_images_prefix + surface_type + '.csv'))
            # query_images_list = query_images_list[query_images_list['true_label'] == surface_quality]
            # query_images = df[df['image_id'].isin(query_images_list['image_id'].astype(str))].reset_index(drop=True)
            # if len(query_images) < len(query_images_list):
            #     missing_ids = []
            #     print(f'Query images are not completely represented in dataset {query_dataset}:\n{missing_ids}')
            # query_embeddings = query_embeddings[df['image_id'].isin(query_images['image_id'])].to(device)
            # print(query_images.groupby('type').size())

            # TODO: check if embeddings to device
            ...
            while True:
                ...
                if is_threshold_given:
                    df = pd.read_csv(os.path.join(experiments_path, threshold_experiment_name))
                    ...
                    search_results.to_csv(os.path.join(experiments_path, experiment_file))      
                    ...
                    for seed in [42, 1000, 3]:
                        ...
                        search_results['searched_quality_predicted'] = (search_results['score'] > optimal_threshold).astype(str)
                        search_results.to_csv(os.path.join(experiments_path, f'seed{seed}_{experiment_file}'))

                    plot_name = os.path.join(experiments_path, f'ROC_{embeddings_model}_{surface_type}_{surface_quality}.png')
                    plot_roc_curve(tprs_list, fprs_list, surface_type, surface_quality, plot_name)

                    threshold_results = pd.DataFrame(results_list)
                    threshold_results.to_csv(os.path.join(experiments_path, threshold_experiment_name)) 
