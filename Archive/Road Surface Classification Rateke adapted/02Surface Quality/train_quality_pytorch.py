...
quality_label_list = ["asphalt", "concrete", "paving_stones", "sett", "unpaved"]
max_indeces = [3, 3, 3, 2, 2]

wandb.login()

for index, quality_label in enumerate(quality_label_list):

    train_path = os.path.join(execution_directory, subdirectory_name, quality_label)
    ...
    wandb.init(
        ...
        name=f"quality {quality_label} {run}",
        # track hyperparameters and run metadata
        config={
            "Surface Type": quality_label,
    ...
    max_index = max_indeces[
        index
    ]  # maximum index we want to read, here we have 4 classes
    ...
    def train(num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            avg_accuracy = accuracy_total / len(valid_loader)

            print(
                "Epoch [{}/{}], Train Loss: {:.4f}, Validation Loss: {:.4f}, Validation Accuracy: {:.2f}%".format(
                    epoch + 1, num_epochs, avg_train_loss, avg_val_loss, avg_accuracy
                )
            )
            wandb.log(
                {
                    ...
                    "average validation accuracy": avg_accuracy,
                }
            )
    ...
    torch.save(model.state_dict(), f"pytorch_CNN_{quality_label}")
    # wandb.save("pytorch_CNN")
    wandb.finish()
