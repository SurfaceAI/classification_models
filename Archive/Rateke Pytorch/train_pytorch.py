...
train_path = os.path.join(
    execution_directory, "training_data/annotated_images_downsampled_without_concrete"
)
...
train_data, valid_data = preprocessing.train_validation_spilt_datasets(
    train_path, validation_size, train_transform, valid_transform, random_state=seed
)
...
def train(num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # if i % 100 == 0:
        print(
            "Epoch [{}/{}], Train Loss: {:.4f}, Train Accuracy: {:.2f}%, Validation Loss: {:.4f}, Validation Accuracy: {:.2f}%".format(
                epoch + 1,
                num_epochs,
                avg_train_loss,
                avg_train_accuracy,
