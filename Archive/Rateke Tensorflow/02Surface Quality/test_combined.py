...
with graph.as_default():
    saver = tf.compat.v1.train.import_meta_graph(
        f"{execution_directory}/roadsurface-model.meta"
    )
    # Acessing the graph
    #
...
saver.restore(
    sess, tf.train.latest_checkpoint(f"{execution_directory}/typeCheckpoint/")
)
...
with graphAQ.as_default():
    saverAQ = tf.compat.v1.train.import_meta_graph(
        f"{execution_directory}/roadsurfaceAsphaltQuality-model.meta"
    )
    # Acessing the graph
    #
    ...
    y_test_imagesAQ = np.zeros(
        (1, len(os.listdir(f"{execution_directory}/training_data_asphalt_quality")))
    )

sessAQ = tf.compat.v1.Session(graph=graphAQ)
saverAQ.restore(
    sessAQ, tf.train.latest_checkpoint(f"{execution_directory}/asphaltCheckpoint/")
)
...
with graphPQ.as_default():
    saverPQ = tf.compat.v1.train.import_meta_graph(
        f"{execution_directory}/roadsurfacePavedQuality-model.meta"
    )
    # Acessing the graph
    #
    ...
    y_test_imagesPQ = np.zeros(
        (1, len(os.listdir(f"{execution_directory}/training_data_paved_quality")))
    )

sessPQ = tf.compat.v1.Session(graph=graphPQ)
saverPQ.restore(
    sessPQ, tf.train.latest_checkpoint(f"{execution_directory}/pavedCheckpoint/")
)
